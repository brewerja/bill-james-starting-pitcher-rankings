from sqlmodel import Session, SQLModel, create_engine, select, func, cast, Integer, update, case
...
with Session(engine) as session:
    ...
    conditional_sum = case(
        (row_number >= 100, sum_last_100),
        else_=None
    ).label("runs_at_venue_last_100_days")

    subquery = (
        select(Game.id.label("game_id"), conditional_sum)
        .subquery()
    )
